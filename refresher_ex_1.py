...
# your code here
sentences = s.split(".") #Trigger split at the .
print("There are", len(sentences), "sentences:") #len counts the number of sentences created by the split
for x in sentences: # x becomes the individual sentences created by the split
    print(x) # prints each sentence x until all sentences have been printed
...
# your code here
s = "Python is an interpreted, high-level, general-purpose programming language."
words = s.split(" ") #Triggers splits at spaces
upper_case = False
for y in words:
    y = y.replace("."," ") #Replaces . with a space
    y = y.replace(","," ") #Replaces , with a space
    if upper_case == True:
        print(y.upper())
        upper_case = False
        ...
        print(y)
        upper_case = True  #Makes every other word uppercase by flipping upper_case between false and true 
...
for x in range(5,15):
    if x * 2 + len(filler) > len(s):
        ...
        abb_s = s[0:x] + filler + s[-x:] # slice off ends and glue together with filler chars
        print(x, abb_str)
